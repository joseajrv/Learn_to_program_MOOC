...
def is_valid_word(wordlist, word):
    ...
    True
    >>> is_valid_word(['ANT', 'BOX', 'SOB', 'TO'], 'SOV')
    False
    """
    if word in wordlist:
        return True
...
def make_str_from_row(board, row_index):
    ...
    for i in range(len(board[row_index])):
        str_from_row = str_from_row + board[row_index][i]
    return str_from_row
...
def board_contains_word(board, word):
    ...
    True
    >>> board_contains_word([['A', 'N', 'T', 'T'], ['X', 'S', 'O', 'B']], 'SOV')
    False
...
def word_score(word):
    ...
    if len(word) < 3:
        ...
        points_per_char = 3
    return len(word) * points_per_char
...
"""
Created on Tue Dec  5 00:23:45 2017
